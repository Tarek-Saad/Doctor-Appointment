...
const login = async(req, res) => {
    try {
        // التأكد من وجود الإيميل في قاعدة البيانات
        const emailPresent = await User.findOne({ email: req.body.email });
        if (!emailPresent) {
            console.log("Email not found"); // لتوضيح السبب
            return res.status(400).send("Incorrect credentials");
        }
        console.log("Email found:", emailPresent.email);

        try {
            // التأكد من تطابق الدور
            if (emailPresent.role !== req.body.role) {
                console.log("Role mismatch: expected", req.body.role, "but found", emailPresent.role);
                return res.status(404).send("Role does not exist");
            }
            console.log("Role matched:", emailPresent.role);
        } catch (roleError) {
            console.error("Error during role check:", roleError);
        ...
        try {
            // إنشاء التوكن JWT
            const token = jwt.sign({ userId: emailPresent._id, isAdmin: emailPresent.isAdmin, role: emailPresent.role },
        ...
        console.error("Error during login:", error); // طباعة الخطأ بالكامل في الـ console
...
const updateprofile = async(req, res) => {
    try {
        const hashedPass = await bcrypt.hash(req.body.password, 10);
        const result = await User.findByIdAndUpdate({ _id: req.locals }, {...req.body, password: hashedPass });
        if (!result) {
